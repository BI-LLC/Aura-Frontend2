...
-- Assistant voice preferences - Maps assistant keys to ElevenLabs voice IDs
CREATE TABLE IF NOT EXISTS assistant_voice_prefs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    assistant_key VARCHAR(255) NOT NULL, -- e.g., 'bib_halder', 'john-doe'
    tenant_id UUID REFERENCES tenants(tenant_id) ON DELETE CASCADE,
    provider VARCHAR(50) DEFAULT 'elevenlabs', -- voice provider (elevenlabs, openai, etc.)
    voice_id VARCHAR(255) NOT NULL, -- ElevenLabs voice ID
    model VARCHAR(100) DEFAULT 'eleven_turbo_v2', -- voice model
    params JSONB DEFAULT '{}', -- voice parameters (stability, similarity_boost, etc.)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(assistant_key, tenant_id) -- One voice config per assistant per tenant
);

-- Index for fast lookups
CREATE INDEX IF NOT EXISTS idx_assistant_voice_prefs_key ON assistant_voice_prefs(assistant_key);
