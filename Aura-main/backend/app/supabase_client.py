...
class SupabaseClient:
    ...
    async def get_assistant_voice_prefs(self, assistant_key: str, tenant_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get voice preferences for an assistant"""
        try:
            query = self.client.table("assistant_voice_prefs").select("*").eq("assistant_key", assistant_key)
            if tenant_id:
            ...
            logger.error(f"Error getting assistant voice preferences: {e}")
            return None
    
    async def create_assistant_voice_prefs(self, voice_data: Dict[str, Any]) -> Dict[str, Any]:
        """Create or update assistant voice preferences"""
        try:
            result = self.client.table("assistant_voice_prefs").upsert(voice_data).execute()
            return result.data[0] if result.data else None
        except Exception as e:
            logger.error(f"Error creating assistant voice preferences: {e}")
            raise
